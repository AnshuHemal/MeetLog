  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (activeTab === "link") {
      ...
      try {
        ...
        if (isYtLink) {
          addLog("info", "pipeline", `Starting YouTube audio extraction for "${title}"`);
          addLog("storage", "youtube", `Querying YouTube video streams: ${mediaUrl.slice(0, 55)}...`);
          addLog(
            "info",
            "pipeline",
            "Transcoding video to 16kHz speech MP3 via FFmpeg engine..."
          );
        } else {
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                            ...
                            {previewData.platform === "youtube"
                              ? "Audio will be extracted via FFmpeg & transcribed with speaker diarization"

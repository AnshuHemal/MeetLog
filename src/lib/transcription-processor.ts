...
import { getSarvamJobStatus, downloadSarvamJobTranscript } from "@/lib/sarvam";
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (outputs.length > 0) {
      ...
      if (jsonOutput) {
        ...
        const stabilizeMs = attempt <= 2 ? 10_000 : 5_000;
        await updateProgressMessage(
          meetingId,
  ...
  await prisma.$transaction(async (tx) => {
    try {
      await tx.meeting.update({
        where: { id: meetingId, status: "TRANSCRIBING" },
    ...
    await tx.speakerLabel.deleteMany({ where: { meetingId } });
  });

  const batchSize = 200;
  for (let i = 0; i < entries.length; i += batchSize) {
    const chunk = entries.slice(i, i + batchSize);
    const segmentsToInsert = chunk.map((seg, idx) => ({
      meetingId,
      speakerId: normalizeSpeakerId(seg.speaker_id),
      startTime: parseFloat(seg.start_time_seconds.toString()),
      endTime: parseFloat(seg.end_time_seconds.toString()),
      text: seg.transcript,
      index: i + idx,
    }));

    await prisma.transcriptSegment.createMany({ data: segmentsToInsert });
  }

  const uniqueSpeakers = collectUniqueSpeakerIds(entries);
  if (uniqueSpeakers.length > 0) {
    await prisma.speakerLabel.createMany({
      data: uniqueSpeakers.map((speakerId, idx) => ({
        meetingId,
        speakerId,
...
async function generateAIInsights(
  ...
  const transcriptText = entries
    .map((seg) => `${normalizeSpeakerId(seg.speaker_id)}: ${seg.transcript}`)
    .join("\n");

  let summaryMarkdown =
    "### Meeting Transcription Summary\n\nAI summary generation is disabled or key is missing. The transcript is available in the main panel.";
  let chaptersJson: string | null = null;

  try {
    await updateProgressMessage(meetingId, "Running Gemini AI: Analyzing transcript and extracting action checklist...");
    ...
    log(`Warning: AI insights failed: ${err.message}`);
  }

  try {
    await updateProgressMessage(meetingId, "Running Gemini AI: Segmenting meeting chapters and timelines...");
    const chapters = await generateMeetingChapters(transcriptText);
    chaptersJson = JSON.stringify(chapters);
  } catch (err: any) {
    log(`Warning: Chapter generation failed: ${err.message}`);
  }

  await updateProgressMessage(meetingId, "Saving meeting insights to database...");
  await prisma.meeting.update({
    where: { id: meetingId },
    data: {
      summaryMarkdown,
...
export async function markAsFailed(meetingId: string, errorMessage: string) {
  ...
  try {
    ...
    log(`Warning: Failed to send dead-letter email for ${meetingId}: ${err.message}`);
  }
}
